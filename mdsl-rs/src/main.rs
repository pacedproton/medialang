...
fn test_neo4j(filename: &str, options: &[String]) {
    ...
    // Parse options; falls back to NEO4J_URL, then to a local server
    let from_env = std::env::var("NEO4J_URL").ok();
    let mut neo4j_url = from_env.as_deref().unwrap_or("http://localhost:7474");
    for option in options {
