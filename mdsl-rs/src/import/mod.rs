...
impl SqlImporter {
    ...
    pub async fn generate_complete_mdsl(&self) -> Result<String> {
        ...
        let mut mo_constant_data = self.extract_media_outlets_full(&connection).await?;
        ...
        // Ensure every outlet ID referenced by relationships is declared as an OUTLET
        {
            use std::collections::HashSet;

            let declared_ids: HashSet<String> =
                mo_constant_data.iter().map(|o| o.id_mo.clone()).collect();

            let referenced_ids: HashSet<String> = relationships_data
                .iter()
                .flat_map(|r| [r.source_id.clone(), r.target_id.clone()])
                .filter(|id| !id.is_empty() && id != "NULL")
                .collect();

            let missing: Vec<String> = referenced_ids.difference(&declared_ids).cloned().collect();

            if !missing.is_empty() {
                println!(
                    "ℹ️ Adding {} stub OUTLET declarations for IDs only referenced in links",
                    missing.len()
                );
            }

            for id in missing {
                mo_constant_data.push(MediaOutletData {
                    id_mo: id.clone(),
                    title: format!("Unknown Outlet {}", id),
                    sector: "90".to_string(), // Multimedia/Unknown
                    mandate: "1".to_string(),
                    location: "KA".to_string(),
                    distribution_area: "20".to_string(),
                    local: "0".to_string(),
                    ...
                    editorial_line_start: "".to_string(),
                    editorial_line_end: "".to_string(),
                    comments:
                        "Stub autogenerated for relationship target/source without base record"
                            .to_string(),
                });
            }
        }
        ...
        // Generate media outlet units and families (now includes stubs for referenced-only IDs)
    ...
    fn generate_mdsl_header(&self, output: &mut String) -> Result<()> {
        ...
        // Database name only: host and credentials stay in the environment and
        // must not be written into generated artefacts.
        writeln!(
            output,
            "// Database: {}",
            self.config
    ...
    fn generate_data_blocks_mdsl(
        ...
        for data in market_data {
            if data.id_mo != "NULL"
                && !data.id_mo.is_empty()
                && valid_outlet_ids.contains(&data.id_mo)
            {
                data_by_outlet
