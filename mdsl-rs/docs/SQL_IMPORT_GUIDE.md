...
Copy `.env.example` to `.env` in the repository root and set `DATABASE_URL`:

```
DATABASE_URL=postgresql://user:password@localhost/media_db
```

`.env` is git-ignored. Every subcommand reads `DATABASE_URL` when `--connection`
is not given, which keeps credentials out of the repository and out of shell
history. `--connection` still works and takes precedence, as does a `DATABASE_URL`
already exported in the environment.

Characters reserved in a URI (`: / ? # [ ] @ , ; = %`) must be percent-encoded in
the password field.

### 2. Test Database Connection

```bash
./target/debug/sql_import test
```
...
```bash
./target/debug/sql_import analyze
```
