...
The dataset is curated and archived in MediaLanguage (MDSL), a purpose-built domain-specific language with a publicly available Rust compiler. The canonical deposit preserves typed relations, temporal semantics, provenance, and aggregation rules as first-class constructs; relational or graph views can be derived on demand. The modular MDSL package is publicly available on figshare (\url{https://doi.org/10.6084/m9.figshare.31908523}), with root file \texttt{anmi\_main.mdsl}. Compiler source code is available at \url{https://github.com/pacedproton/medialang}.
