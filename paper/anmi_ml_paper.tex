...
The ANMI media-supply release is deposited as a single figshare item organized into three complementary representations of the same logical corpus, accompanied by documentation. The repository is organized as follows:

\begin{verbatim}
anmi-media-v1/
  01_relational/                      (PostgreSQL source views)
    pg_dump_graphv3_full.sql            schema + data, restorable via psql
    pg_dump_graphv3_full_schema.sql     DDL only (human-readable)
  02_csv_tables/                      (flat per-table exports, UTF-8)
    mo_constant.csv                     657 outlet constant attributes
    mo_year.csv                         outlet-year market observations
    sources_names.csv                   51 structured source records
    nd_company.csv, nd_person.csv,
    nd_ownership.csv, nd_crawl_log.csv  ownership and provenance tables
    11_succession.csv, 12_amalgamation.csv,
    13_new_distribution_area.csv,
    14_new_sector.csv, 19_interruption.csv,
    21_split_off.csv, 22_offshoot.csv,
    23_merger.csv                       8 diachronic relation tables
    31_main_media_outlet.csv,
    33_umbrella.csv, 34_collaboration.csv   3 synchronic relation tables
  03_mdsl/                            (canonical MDSL source package)
    ...
    sources.mdsl
    anmi_full_export.mdsl               bulk export of all 657 outlets with
                                        identity, lifecycle, and metadata
    kronenzeitung/                      curated hand-authored example family
      kronen_zeitung_freeze3.mdsl
      beziehungen.mdsl
\end{verbatim}

The three representations are equivalent in substantive content but differ in affordances. The \textbf{relational} layer (\texttt{01\_relational/}) is a direct \texttt{pg\_dump} of the ANMI PostgreSQL source schema and is the most faithful to the operational record: it preserves all tables, indexes, and constraints in their native form. The \textbf{tabular} layer (\texttt{02\_csv\_tables/}) provides each table as a UTF-8 CSV with a header row, enabling direct use in R, Python, DuckDB, or spreadsheet tools without a database. The \textbf{MDSL} layer (\texttt{03\_mdsl/}) is the canonical archival form: it encodes typed relations, temporal intervals, provenance, date precision, and aggregation constraints as first-class language constructs that cannot be represented faithfully in flat tables or even in a normalized relational schema without semantic loss. The MDSL layer is the authoritative source against which the relational and tabular layers are validated.

Publishing all three forms simultaneously serves the FAIR principles of Interoperability and Reusability directly: readers can choose the representation that matches their tooling and research question rather than being forced to install a compiler or a database to work with the data. At the same time, the MDSL source ensures that analysts who need the full relational semantics---typed diachronic and synchronic link declarations, provenance-linked observation blocks, vocabulary-controlled fields---can work with the complete data model without relying on lossy derivative views.

\subsection{MDSL source file contents}

The MDSL layer of the deposit has two entry points. The \textbf{modular package} is rooted at \texttt{anmi\_main.mdsl}, which chains \texttt{IMPORT} declarations across shared vocabularies (\texttt{anmi\_common\_codes}, \texttt{anmi\_media\_sectors}, \texttt{anmi\_mandate\_types}, \texttt{anmi\_source\_references}), market-data and core-entity schemas, outlet and undertaking unit definitions, four diachronic and synchronic relation modules, and the source catalog. This is the recommended entry point for readers and for compiler validation. The \textbf{bulk export}, \texttt{anmi\_full\_export.mdsl} (25{,}247 lines), is a single-file compilation generated directly from the ANMI PostgreSQL source by the compiler's \texttt{sql\_import} stage; it contains all 657 outlets as \texttt{UNIT} blocks with \texttt{IDENTITY}, \texttt{LIFECYCLE}, and \texttt{METADATA} sub-blocks, plus outlet-level data blocks, and serves as a human-readable counterpart to the relational dump in \texttt{01\_relational/}. A curated example family under \texttt{kronenzeitung/} demonstrates hand-authored MDSL including explicit synchronic link declarations between related outlets; it is not imported by the root but is validated independently. Table~\ref{tab:objects} summarizes MDSL constructs and counts for the full media-supply corpus.
...
\begin{table}[t]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{0.28\linewidth}r>{\raggedright\arraybackslash}p{0.44\linewidth}}
    ...
    \texttt{IMPORT} & 13 & Chained from \texttt{anmi\_main.mdsl} across vocabularies, entity unit definitions, diachronic and synchronic relation schemas, and the source catalog \\
    \bottomrule
  \end{tabular}
  \caption{MDSL constructs in the full ANMI media-supply corpus as expressed in the modular package rooted at \texttt{anmi\_main.mdsl}.}
  \label{tab:objects}
\end{table}

The diachronic and synchronic link declarations live in the four dedicated relation modules (\texttt{MedienangeboteDiachroneBeziehungen.mdsl}, \texttt{MedienangeboteSynchroneBeziehungen.mdsl}, and the two analogous undertaking-relation modules), not in the bulk outlet export. Readers who need the complete corpus including all typed relations should therefore start from the modular entry point \texttt{anmi\_main.mdsl}; the bulk \texttt{anmi\_full\_export.mdsl} is intended as a convenient reference for outlet-level attributes only.
...
The ANMI media-supply dataset is publicly available on figshare~\cite{ANMIFigshare2026} as a single item organized into three complementary representations of the same corpus: a relational layer (\texttt{01\_relational/}) containing a full PostgreSQL dump of the source database (schema and data) together with a human-readable schema-only DDL file; a tabular layer (\texttt{02\_csv\_tables/}) containing each table as a UTF-8 CSV with a header row, directly usable in R, Python, DuckDB, or spreadsheet tools without any database software; and the canonical MDSL source package (\texttt{03\_mdsl/}), rooted at the entry point \texttt{anmi\_main.mdsl}, which imports modular files for outlets (\texttt{Medienangebot.mdsl}), undertakings (\texttt{Medienunternehmen.mdsl}), diachronic and synchronic outlet and undertaking relations, sources, and market-data and core-entity schemas, and additionally includes a bulk single-file export (\texttt{anmi\_full\_export.mdsl}) of all 657 outlets with their identity, lifecycle, and metadata sub-blocks. A curated example family under \texttt{kronenzeitung/} demonstrates hand-authored MDSL. The deposit includes accompanying documentation (codebook, MDSL reference guide, manifest, and readme) as released on the record. Persistent identifier: \url{https://doi.org/10.6084/m9.figshare.31908523}.
